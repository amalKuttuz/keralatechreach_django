...
function getAllEvents($conn) {
    ...
    while ($event = $eventResult->fetch_assoc()) {
        $response["events"][] = array(
            ...
            "sponsor_level" => $event['sponsor_level'],
...
function getUpdatedEvents($conn, $lastUpdate) {
    // Sanitize the input to prevent SQL injection
    $lastUpdate = $conn->real_escape_string($lastUpdate);
    ...
    $categoryQuery = "SELECT category_id, category, created_at 
                      FROM event_category 
    ...
    if ($categoryResult) {
        ...
        error_log("Error in category query: " . $conn->error);
    }
    
    // Fetch updated Districts
    $districtQuery = "SELECT district_id, district, created_at 
                     FROM districts 
                     WHERE created_at > '$lastUpdate'";
    $districtResult = $conn->query($districtQuery);
    if ($districtResult) {
        ...
        error_log("Error in district query: " . $conn->error);
    }
    
    // Fetch updated Events with Sponsor Data
    $eventQuery = "SELECT e.*,
                  c.category as category_name,
                  d.district as district_name
                  FROM events e
    ...
    if ($eventResult) {
        ...
        error_log("Error in event query: " . $conn->error);
    }

    // Fetch updated Notes
    $notesQuery = "SELECT * FROM notes WHERE updated_date > '$lastUpdate'";
    $notesResult = $conn->query($notesQuery);
    if ($notesResult) {
        while ($note = $notesResult->fetch_assoc()) {
            $response["notes"][] = array(
        ...
        error_log("Error in notes query: " . $conn->error);
    }
...
function getEventsByCategory($conn, $categoryId) {
    // Sanitize the input to prevent SQL injection
    ...
    // Fetch Events with Sponsor Data for the specified category
    $eventQuery = "SELECT e.*,
        c.category as category_name,
        d.district as district_name
        FROM events e
    ...
    if ($eventResult) {
        ...
        error_log("Error in category events query: " . $conn->error);
    }
